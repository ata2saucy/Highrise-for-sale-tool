#!/usr/bin/env python3
"""
build_workbook.py — deterministic v2 sale-comp workbook generator.
...
the workbook — run this.
    ...
    python build_workbook.py comp_data.json ["Out Name Sale Comps _vACTIVE.xlsx"]

Produces the canonical 6 sheets, in order:
  Output · Building Summary · Data_Summary · RD Resale · RD Pre-Con · Floor Plans
...
GREEN  = "FFE2EFDA"   # confidence: sold+verified
CREAM  = "FFFFF2CC"   # confidence: active / older / caveat / pre-con asking
...
# ---------------------------------------------------------------- RD sheets
RD_HEADERS = ["Include","Sq Ft.","Price","Adj. Price","$/sq ft","Adj. $/sq ft","Adj BD",
 ...
 "Sold Price ($)","List Price ($)","PSF (Calculated)","PSF (Listing)","Days on Market",
 "Maintenance Fee ($/mo)","Property Tax ($/yr)","Sold Date","MLS#","Listing URL","Description",
...
def build_rd(wb, title, rows, product):
    ...
    for i, rec in enumerate(rows):
        ...
        C(ws, f"AN{r}", f"=IFERROR(E{r}*(1+IF(AM{r}=\"Pre-Con\",Data_Summary!$C$3,0)),\"\")", nf=NF_PSF)
        # data (hard) columns
        ...
        C(ws, f"AB{r}", rec.get("sold_price"), nf=NF_INT, font=F_BLUE)
        C(ws, f"AC{r}", rec.get("list_price"), nf=NF_INT, font=F_BLUE)
        C(ws, f"AF{r}", rec.get("days_on_market"))
        C(ws, f"AG{r}", rec.get("maint_fee"))
        C(ws, f"AH{r}", rec.get("property_tax"))
        C(ws, f"AI{r}", rec.get("sold_date"))
    ...
    if not rows:
        C(ws, "K3", f"No pre-construction comps in this set."
                    if product == "Pre-Con" else "No rows.", font=F_GREY)
    return ws
...
BS_HEADERS = ["Building","Address","Area","Developer","First Occupancy","Yr Built","Product",
 "Storeys","Units","Transaction Count","Avg SF","Avg Price","Avg $/SF"]
...
def build_building_summary(wb, subject, buildings):
    ...
    for i, b in enumerate(buildings):
        ...
        C(ws, f"J{r}", f"=COUNTIFS('RD Resale'!$K:$K,$A{r},'RD Resale'!$A:$A,1)")
        C(ws, f"K{r}", f"=IFERROR(AVERAGEIFS('RD Resale'!$B:$B,'RD Resale'!$K:$K,$A{r},'RD Resale'!$A:$A,1),\"\")", nf=NF_INT)
        C(ws, f"L{r}", f"=IFERROR(AVERAGEIFS('RD Resale'!$AB:$AB,'RD Resale'!$K:$K,$A{r},'RD Resale'!$A:$A,1),\"\")", nf=NF_INT)
        C(ws, f"M{r}", f"=IFERROR(AVERAGEIFS('RD Resale'!$E:$E,'RD Resale'!$K:$K,$A{r},'RD Resale'!$A:$A,1),\"\")",
          nf=NF_PSF, fill=FILL_REC)
    nb = len(buildings)
    rc = r0 + nb         # ALL RESALE
    ra = r0 + nb + 1     # ALL PRE-CON
    C(ws, f"A{rc}", "ALL RESALE", fill=FILL_BAND)
    C(ws, f"J{rc}", "=SUM(J%d:J%d)" % (r0, r0+nb-1), fill=FILL_BAND)
    C(ws, f"K{rc}", "=IFERROR(AVERAGEIFS('RD Resale'!$B:$B,'RD Resale'!$A:$A,1),\"\")", nf=NF_INT, fill=FILL_BAND)
    C(ws, f"L{rc}", "=IFERROR(AVERAGEIFS('RD Resale'!$AB:$AB,'RD Resale'!$A:$A,1),\"\")", nf=NF_INT, fill=FILL_BAND)
    C(ws, f"M{rc}", "=IFERROR(AVERAGEIFS('RD Resale'!$AN:$AN,'RD Resale'!$A:$A,1),\"\")", nf=NF_PSF, fill=FILL_BAND)
    C(ws, f"A{ra}", "ALL PRE-CON", fill=FILL_BAND)
    C(ws, f"J{ra}", "=COUNTIFS('RD Pre-Con'!$A:$A,1)", fill=FILL_BAND)
    C(ws, f"K{ra}", "=IFERROR(AVERAGEIFS('RD Pre-Con'!$B:$B,'RD Pre-Con'!$A:$A,1),\"\")", nf=NF_INT, fill=FILL_BAND)
    C(ws, f"L{ra}", "=IFERROR(AVERAGEIFS('RD Pre-Con'!$AB:$AB,'RD Pre-Con'!$A:$A,1),\"\")", nf=NF_INT, fill=FILL_BAND)
    C(ws, f"M{ra}", "=IFERROR(AVERAGEIFS('RD Pre-Con'!$AN:$AN,'RD Pre-Con'!$A:$A,1),\"\")", nf=NF_PSF, fill=FILL_BAND)
    return ws
...
def build_data_summary(wb, d):
    ...
    pa = lev["parking_adj"]; pc = lev["precon_premium"]
    smo = lev["subject_mix_output"]; smm = lev["subject_mix_manual"]

    # ---- levers C1:C4
    C(ws, "B1", "Date Filter (include sales on/after)")
    C(ws, "C1", dt(lev["date_filter"]), nf=NF_DATE, font=F_BLUE, fill=FILL_YEL)
    C(ws, "D1", "lever — trailing window; sales on/after this date count. Tunable.", font=F_GREY)
    C(ws, "B2", "Parking Adjustment ($/spot)")
    C(ws, "C2", pa["value"], nf=NF_USD2, font=F_BLUE, fill=FILL_YEL)
    C(ws, "D2", pa["note"], font=F_GREY)
    C(ws, "B3", "Pre-con premium (pre-con comps only)" + (" — OFF" if pc.get("off") else ""))
    C(ws, "C3", pc["value"], nf=NF_PCT, font=F_BLUE, fill=FILL_YEL)
    C(ws, "D3", pc["note"], font=F_GREY)
    sp = lev.get("subject_premium")
    fb = sp["value"] if sp else 0
    C(ws, "B4", "Subject premium (new-build / launch; comp $/SF → subject)")
    ...
    for col, t in zip("ABCDE", ["Beds","Incl.","Avg SF","Avg Price","Avg $/SF"]):
        C(ws, f"{col}19", t, font=F_HDR, fill=FILL_HDR)
    for r, bed in [(20,"1"),(21,"2"),(22,"3")]:
        C(ws, f"A{r}", f"{bed} Bed")
        C(ws, f"B{r}", f"=COUNTIFS('RD Resale'!$H:$H,\"{bed}\",'RD Resale'!$A:$A,1)")
        C(ws, f"C{r}", f"=IFERROR(AVERAGEIFS('RD Resale'!$B:$B,'RD Resale'!$H:$H,\"{bed}\",'RD Resale'!$A:$A,1),\"\")", nf=NF_INT)
        C(ws, f"D{r}", f"=IFERROR(AVERAGEIFS('RD Resale'!$AB:$AB,'RD Resale'!$H:$H,\"{bed}\",'RD Resale'!$A:$A,1),\"\")", nf=NF_INT)
        C(ws, f"E{r}", f"=IFERROR(AVERAGEIFS('RD Resale'!$AN:$AN,'RD Resale'!$H:$H,\"{bed}\",'RD Resale'!$A:$A,1),\"\")", nf=NF_PSF)

    # ---- By Bedroom — primary only (rows 24-28)
    C(ws, "A24", f"By Bedroom — {primary} only (new-build basis)", font=F_BOLD)
    for col, t in zip("ABCDEF", ["Beds","Incl.","Avg SF","Avg Price","$/SF","Subj Price (+prem)"]):
        C(ws, f"{col}25", t, font=F_HDR, fill=FILL_HDR)
    for r, bed in [(26,"1"),(27,"2"),(28,"3")]:
        C(ws, f"A{r}", f"{bed} Bed")
        C(ws, f"B{r}", f"=COUNTIFS('RD Resale'!$K:$K,\"{primary}\",'RD Resale'!$H:$H,\"{bed}\",'RD Resale'!$A:$A,1)")
        C(ws, f"C{r}", f"=IFERROR(AVERAGEIFS('RD Resale'!$B:$B,'RD Resale'!$K:$K,\"{primary}\",'RD Resale'!$H:$H,\"{bed}\",'RD Resale'!$A:$A,1),\"\")", nf=NF_INT)
        C(ws, f"D{r}", f"=IFERROR(AVERAGEIFS('RD Resale'!$AB:$AB,'RD Resale'!$K:$K,\"{primary}\",'RD Resale'!$H:$H,\"{bed}\",'RD Resale'!$A:$A,1),\"\")", nf=NF_INT)
        C(ws, f"E{r}", f"=IFERROR(AVERAGEIFS('RD Resale'!$E:$E,'RD Resale'!$K:$K,\"{primary}\",'RD Resale'!$H:$H,\"{bed}\",'RD Resale'!$A:$A,1),\"\")", nf=NF_PSF)
    ...
    C(ws, "A33", "Subject premium");                       C(ws, "C33", "=C4", nf=NF_PCT)
    ...
    C(ws, "C58", f"=IFERROR(AVERAGEIFS('RD Resale'!$E:$E,'RD Resale'!$K:$K,\"{primary}\",'RD Resale'!$A:$A,1),\"\")", nf=NF_PSF2)
    C(ws, "A59", "All-comp avg $/SF"); C(ws, "C59", "=IFERROR(AVERAGEIFS('RD Resale'!$AN:$AN,'RD Resale'!$A:$A,1),\"\")", nf=NF_PSF2)
    C(ws, "A60", f"Oldest comp ({oldest}) avg $/SF")
    C(ws, "C60", f"=IFERROR(AVERAGEIFS('RD Resale'!$E:$E,'RD Resale'!$K:$K,\"{oldest}\",'RD Resale'!$A:$A,1),\"\")", nf=NF_PSF2)
    ...
    C(ws, "A67", "1-Bed (H=1, den-incl)"); C(ws, "B67", "=COUNTIFS('RD Resale'!$H:$H,\"1\",'RD Resale'!$A:$A,1)")
    C(ws, "C67", "=IFERROR(B67/$B$70,\"\")", nf=NF_PCT); C(ws, "D67", "=H2", nf=NF_PCT)
    C(ws, "A68", "2-Bed (H=2, incl 2+den)"); C(ws, "B68", "=COUNTIFS('RD Resale'!$H:$H,\"2\",'RD Resale'!$A:$A,1)")
    C(ws, "C68", "=IFERROR(B68/$B$70,\"\")", nf=NF_PCT); C(ws, "D68", "=H3", nf=NF_PCT)
    C(ws, "A69", "3-Bed (H=3)"); C(ws, "B69", "=COUNTIFS('RD Resale'!$H:$H,\"3\",'RD Resale'!$A:$A,1)")
    C(ws, "C69", "=IFERROR(B69/$B$70,\"\")", nf=NF_PCT); C(ws, "D69", "=H4", nf=NF_PCT)
    C(ws, "A70", "Total included"); C(ws, "B70", "=B67+B68+B69")
    C(ws, "A71", "1+Den share (Q=1+1)"); C(ws, "B71", "=COUNTIFS('RD Resale'!$Q:$Q,\"1+1\",'RD Resale'!$A:$A,1)")
    C(ws, "C71", "=IFERROR(B71/$B$70,\"\")", nf=NF_PCT); C(ws, "D71", "=T4", nf=NF_PCT)

    # ---- PRE-CON PREMIUM BASIS (rows 74-75)
    has_pc = len(d.get("precon_rows", [])) > 0
    C(ws, "A74", "PRE-CON PREMIUM BASIS (C3)" + ("" if has_pc else " — N/A this set"), font=F_BOLD)
    if has_pc:
        C(ws, "A75", "Derive C3 from resale ÷ pre-con $/SF spread over comparable vintages:")
        C(ws, "B75", "=IFERROR(AVERAGEIFS('RD Resale'!$E:$E,'RD Resale'!$A:$A,1)/AVERAGEIFS('RD Pre-Con'!$E:$E,'RD Pre-Con'!$A:$A,1)-1,\"\")", nf=NF_PCT)
    else:
        C(ws, "A75", "No pre-construction comps selected, so C3 is unused (0). If pre-con "
                     "comps are added, derive C3 from the resale ÷ pre-con $/SF spread or cite a "
                     "named market study — never a bare %.", font=F_GREY)

    # ---- comp building rollup (placed at bottom; display-only, not cross-referenced)
    base = 78
    C(ws, f"A{base}", "Comp building rollup (included sales)", font=F_BOLD)
    for col, t in zip("ABCDEFG", ["Building","Address","Built","Incl.","Avg SF","Avg Price","Avg $/SF"]):
    ...
    for i, b in enumerate(blds):
        ...
        C(ws, f"D{r}", f"=COUNTIFS('RD Resale'!$K:$K,$A{r},'RD Resale'!$A:$A,1)")
        C(ws, f"E{r}", f"=IFERROR(AVERAGEIFS('RD Resale'!$B:$B,'RD Resale'!$K:$K,$A{r},'RD Resale'!$A:$A,1),\"\")", nf=NF_INT)
        C(ws, f"F{r}", f"=IFERROR(AVERAGEIFS('RD Resale'!$AB:$AB,'RD Resale'!$K:$K,$A{r},'RD Resale'!$A:$A,1),\"\")", nf=NF_INT)
        C(ws, f"G{r}", f"=IFERROR(AVERAGEIFS('RD Resale'!$E:$E,'RD Resale'!$K:$K,$A{r},'RD Resale'!$A:$A,1),\"\")", nf=NF_PSF)
    ...
    C(ws, f"E{rall}", "=IFERROR(AVERAGEIFS('RD Resale'!$B:$B,'RD Resale'!$A:$A,1),\"\")", nf=NF_INT, fill=FILL_BAND)
    C(ws, f"F{rall}", "=IFERROR(AVERAGEIFS('RD Resale'!$AB:$AB,'RD Resale'!$A:$A,1),\"\")", nf=NF_INT, fill=FILL_BAND)
    C(ws, f"G{rall}", "=IFERROR(AVERAGEIFS('RD Resale'!$AN:$AN,'RD Resale'!$A:$A,1),\"\")", nf=NF_PSF, fill=FILL_BAND)
    return ws
...
def col_off(anchor, n):
    """anchor column letter offset by n (0..3): count/size/price/psf."""
...
def build_output(wb, d):
    ...
    primary = subj["primary_building"]; rd = "'RD Resale'"

    C(ws, "B2", f"{subj['title_full']} — SALE COMP SUMMARY (Resale view)", font=F_TITLE)
    for col, t in zip("BCDEFGHI", ["Building","First Occupancy","Avg. Sold Date","Days on Market",
                                   "Transaction Count","Avg Suite Size","Adj. Avg. Sold Price","Avg. Net Sold PSF"]):
    ...
    C(ws, f"B{prem}", "Pre-Con Premium"); C(ws, f"H{prem}", "=Data_Summary!C4", nf=NF_PCT); C(ws, f"I{prem}", "=Data_Summary!C4", nf=NF_PCT)
    C(ws, f"B{impl}", "Implied Untrended Price", fill=FILL_LBLUE)
    ...
    C(ws, f"B{subjr}", "Subject Site (Untrended Price)", fill=FILL_CREAM)
    ...
    C(ws, f"B{wprem}", "Pre-Con Premium"); C(ws, f"H{wprem}", "=Data_Summary!C4", nf=NF_PCT); C(ws, f"I{wprem}", "=Data_Summary!C4", nf=NF_PCT)
    C(ws, f"B{wimpl}", "Implied Untrended Price", fill=FILL_LBLUE)
    ...
    def bed_formulas(row, anc, key, kcol, extra=""):
        # count/size/price/psf for a bed group; kcol is H (beds num) or Q (beds str)
    ...
    n_resale = len(d["resale_rows"]); last = n_resale + 1
    ...
    C(ws, "K35", "Pre-Con Premium")
    for _, anc, _, _ in GRP:
        C(ws, f"{col_off(anc,3)}35", "=Data_Summary!$C$4", nf=NF_USD2)
    C(ws, "K36", "Weighted Avg. (×(1+PC))")
    for _, anc, _, _ in GRP:
        price = col_off(anc,2); psf = col_off(anc,3)
        C(ws, f"{price}36", f"=IFERROR({price}31*(1+${psf}$35),\"\")", nf=NF_USD)
    ...
    C(ws, "K39", "Subject Property (Using Comp $ Price)", font=F_BOLD)
    for _, anc, _, _ in GRP:
        price = col_off(anc,2); C(ws, f"{price}39", f"=IFERROR({price}36,\"\")", nf=NF_USD)
    C(ws, "K40", "Subject Property Price Building Total")
    ...
    C(ws, "K42", "Subject Property (Using Comp PSF Price)")
    for _, anc, _, _ in GRP:
        price = col_off(anc,2); psf = col_off(anc,3); size = col_off(anc,1)
        C(ws, f"{price}42", f"=IFERROR({psf}36*{size}31,\"\")", nf=NF_USD)
    C(ws, "K43", "Subject Property PSF Price Building Total")
    ...
    C(ws, "K45", "Subject Property Price", fill=FILL_LBLUE)
    for _, anc, _, _ in GRP:
        price = col_off(anc,2); C(ws, f"{price}45", f"=IFERROR(AVERAGE({price}39,{price}42),\"\")", nf=NF_USD)
    C(ws, "K46", "Subject Property Price Building Total")
    ...
    C(ws, "K52", "Subject Property Price (Increase vs comps)"); C(ws, "L52", "=IFERROR(L46/L37-1,\"\")", nf=NF_PCT)
    C(ws, "K53", "Subject Property Price (Building Total)", fill=FILL_REG); C(ws, "L53", "=L46")

    # ---- TRREB table
    tr = d.get("trreb", {})
    C(ws, "R56", "Unadjusted"); C(ws, "Z56", "Unadjusted"); C(ws, "AD56", "Unadjusted")
    C(ws, "K57", f"TRREB Data ({tr.get('report_label','')})")
    C(ws, "R57", "Avg Price (1BR)"); C(ws, "Z57", "Avg Price (2BR)"); C(ws, "AD57", "Avg Price (3BR)")
    C(ws, "K58", tr.get("district_label","")); C(ws, "P58", "← TRREB Market Watch (district / City / GTA YoY), read this session", font=F_GREY)
    if tr:
    ...
    C(ws, "B30", "★ RECOMMENDATION — RECOMMENDED SUBJECT PRICE", font=F_BOLD)
    ...
    C(ws, "B34", "Plus — new-build / launch premium"); C(ws, "D34", "=Data_Summary!$C$4", nf=NF_PCT)
    C(ws, "B36", "Recommended price by suite type", font=F_BOLD)
    C(ws, "B37", "Suite", font=F_BOLD); C(ws, "C37", "Avg SF", font=F_BOLD); C(ws, "D37", "Rec. Price", font=F_BOLD)
    ...
    C(ws, "B43", "Implied price @ recommended subject $/SF", font=F_BOLD); C(ws, "D43", "=IFERROR(D42*Data_Summary!$C$34,\"\")", nf=NF_USD)
    C(ws, "B44", "Implied price @ raw comp $/SF (no premium)", font=F_BOLD); C(ws, "D44", "=IFERROR(D42*Data_Summary!$C$31,\"\")", nf=NF_USD)
    C(ws, "B46", "NOTES & CONTEXT", font=F_BOLD)
    notes = subj.get("notes", [])
    notes = notes + ["Confidence fills (Raw Data only): green = sold in-window + SF verified per-unit · "
                     "cream = plate-verified / older / minor caveat / pre-con asking-only · orange = excluded "
...
def build(data, out_path, recalc=True):
    ...
    build_rd(wb, "RD Resale", data["resale_rows"], "Resale")
    build_rd(wb, "RD Pre-Con", data["precon_rows"], "Pre-Con")
    ...
    # workbook order: Output, Building Summary, Data_Summary, RD Resale, RD Pre-Con, Floor Plans
    order = ["Output","Building Summary","Data_Summary","RD Resale","RD Pre-Con","Floor Plans"]
...
if __name__ == "__main__":
    ...
    out = sys.argv[2] if len(sys.argv) > 2 else f"{data['subject']['name']} Sale Comps _vACTIVE.xlsx"
    path = build(data, out)
    print("WROTE", path)
