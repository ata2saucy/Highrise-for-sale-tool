...
Checks, in order:
  ...
  4. COVERAGE   — every sold-in-window unit (sold price present AND date >= C1) either
                  ...
                  with the price but no SF and no record of which routes were tried — FAILS.
...
def structure(path):
    ...
    for s in ("RD Resale", "RD Pre-Con", "Floor Plans"):
...
def tieout(sol, data):
    ...
    import numpy as np
    lev = data["levers"]; C2 = lev["parking_adj"]["value"]; C3 = lev["precon_premium"]["value"]
    df = lev["date_filter"]
    primary = data["subject"]["primary_building"]
    rows = data["resale_rows"]
    def inc(r):
        return bool(r.get("sqft")) and bool(r.get("sold_price")) and str(r.get("date","")) >= df
    E = lambda r: r["sold_price"] / r["sqft"]
    ...
    c59 = avgE(incl)  # AN == E for resale rows
...
def coverage(data):
    """A sold-in-window unit must have an SF or a documented exhaustion record — a silent
    blank (bounded effort masquerading as 'cannot verify') fails. This is the negation of the
    Include predicate on the SF term only: sold price present, date >= C1, but no SF."""
    print("SF-COVERAGE (sold-in-window units: verified SF, or a documented sf_blank)")
    df = data["levers"]["date_filter"]
    REQ = ("condosca_page", "keyplate", "outcome")
    rows = ([("RD Resale", r) for r in data.get("resale_rows", [])]
            + [("RD Pre-Con", r) for r in data.get("precon_rows", [])])
    sold_inwin = [(s, r) for s, r in rows
                  if r.get("sold_price") and str(r.get("date", "")) >= df]
    blanks = [(s, r) for s, r in sold_inwin if not r.get("sqft")]
    undocumented = []
    for s, r in blanks:
        sb = r.get("sf_blank")
    ...
    msg = ("every sold-in-window blank-SF row carries a complete sf_blank "
    ...
    resolved = len(sold_inwin) - len(blanks)
    documented = len(blanks) - len(undocumented)
    ratio = (100.0 * resolved / len(sold_inwin)) if sold_inwin else 100.0
    print(f"    [sold in-window = {len(sold_inwin)}; SF resolved = {resolved} "
...
def main():
    ...
    counts = {"RD Resale": len(data["resale_rows"]) + 1,
              "RD Pre-Con": max(2, len(data["precon_rows"]) + 1),
